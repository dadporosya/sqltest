...
def skill_project(message, projects):
    ...
    if project_name not in projects:
        bot.send_message(message.chat.id, 'Такого проект не существует. Выберите другой проект:', reply_markup=gen_markup(projects))
        bot.register_next_step_handler(message, skill_project, projects=projects)
    else:
...
def set_skill(message, project_name, skills):
    ...
    if skill not in skills:
        bot.send_message(message.chat.id, 'Вы выбрали навык не из списка! Выбери навык', reply_markup=gen_markup(skills))
        bot.register_next_step_handler(message, set_skill, project_name=project_name, skills=skills)
        return
...
if __name__ == '__main__':
    ...
    manager.insert_project([(123456, 'Test Project', 'http://example.com', 1)])
